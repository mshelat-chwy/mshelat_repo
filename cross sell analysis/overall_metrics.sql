set data_date = '2026-04-25';
use schema mkt_sponsored_ads_sandbox;

--select distinct search_term from edldb.mkt_sponsored_ads_sandbox.cross_sell_search_term_mc3_map
...
where a.experiment = '2026_02_SEARCH_CROSS_SELL_CHEWY_ADS_TEAM'
    and a.session_date >= '2026-3-11'
    and a.session_id in (
        select distinct session_id from discovery_sandbox.prd_f_d_expr_search_metrics_optimizely
        where session_date >= '2026-3-11'
            and search_term in (select distinct search_term from edldb.mkt_sponsored_ads_sandbox.cross_sell_search_term_mc3_map)
    )
group by all
order by 3;
